...
namespace Deskhelp_UWP
{
    ...
    sealed partial class App : Application
    {
        ...
        const string timerRegisterTask = "TimerTask1";
        ...
        private void RegisterPushBackgroundTask()
        {
            try
            {
                ...
                showToast("DeskHelp", "Registering Background Services", false);
                ...
                if (!BackgroundTaskRegistration.AllTasks.Any(i => i.Value.Name == timerRegisterTask))
                {
                    ...
                    //builder.TaskEntryPoint = "BackgroundTask.ReRegistrationTask";
        ...
        private void showToast(string title, string content, bool addButtons = true)
        {
        ...
        protected override async void OnBackgroundActivated(BackgroundActivatedEventArgs args)
        {
            ...
            if (args.TaskInstance.Task.Name == timerRegisterTask)
            {
                var result = await validateExpirationDate();
                if ( result == 2 )
                    showToast("Registration Verification", "Registration was renewed");
                else if ( result == 0 )
                    showToast("Registration Verification", "Registration is valid");
                else if ( result == -1 )
                    showToast("Registration Verification", "Error in registration validation");
                deferral.Complete();
                return;
            }
            try
            {
                ...
                {
                    //In - Device 
                    if (payloadJson["notificationType"].ToString().Replace("{", "").Replace("}", "").Replace("\"", "") == "indevice")
                    {

                        var title = payloadJson["title"].ToString().Replace("{", "").Replace("}", "").Replace("\"", "");
                        var content = payloadJson["body"].ToString().Replace("{", "").Replace("}", "").Replace("\"", "");
