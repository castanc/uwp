...
namespace Deskhelp_UWP
{
    ...
    public class userInfo
    {
        ...
        public deviceDetails deviceDeatils { set; get; }
    }
    sealed partial class App : Application
    {
        ...
        {
            CheckUserUrl = "https://tcsdeskhelpdev.azurewebsites.net/notification/checkUsersSubscription",
            UpdateUserUrl = "https://tcsdeskhelpdev.azurewebsites.net/notification/subscribe",
            PublicKey = "BB5nzfoUKxl3DRazM1PD59LxWstFfeffstlyXA5RTT5XMGFD1gMa5xB3MGxHE0t3armwLPSblkjmjATWkV6KEvA",
            JsonTemplate = "{ 'userID':'USERID'}"
        };
        ...
        public App()
        {
            ...
            saveParametersFirstTime("DeskHelpConf.json");
            initializeLocalParameters("DeskHelpConf.json");
            MainPage.dhp = dhp;
            //todo: test
            //renewSubscription();
        ...
        /// <returns></returns>
        private ToastContent showToast(string title, string content, dynamic ButtonArray = null, bool addButtons = true)
        {
            if (addButtons)
            {
                ...
                try
                {
                    ...
                    if (deviceDetails["deviceID"].ToString().Replace("{", "").Replace("}", "").Replace("\"", "") == DeviceInfo)
                    {
                        ...
                        if (inDevice)
                        {
                            ...
                            ToastContent visual = showToast(title, content, buttons, true);
                            ToastNotificationManager.CreateToastNotifier().Show(new ToastNotification(visual.GetXml()));
                        }
                        else if (inApp)
                        {
        ...
        public async void saveParametersFirstTime(string fileName)
        {
            ...
            string data = JsonConvert.SerializeObject(dhp);
            await Windows.Storage.FileIO.WriteTextAsync(sampleFile, data);
        }
        
        private async Task<int> initializeLocalParameters(string fileName)
        {
            int result = 0;
            Windows.Storage.StorageFolder storageFolder = Windows.Storage.ApplicationData.Current.LocalFolder;
            Windows.Storage.StorageFile sampleFile;
            try
            {
                ...
                string data = await Windows.Storage.FileIO.ReadTextAsync(sampleFile);
                dhp = JsonConvert.DeserializeObject<DeskHelpParameters>(data);
            }
            catch (Exception ex )
            {
                sampleFile = await storageFolder.CreateFileAsync(fileName);
